...
4. Respond with ONLY a raw JSON array of events. Do not include markdown code fences, backticks, or any conversational text before or after the JSON.
5. Infer reasonable start times/durations when the user is vague, but never invent specific named entities that weren't in the input.
6. NEVER default an unspecified or vague time to 00:00 (midnight) — that is almost always wrong for a real activity. If the user gives only a vague time-of-day word instead of an exact hour, use these defaults unless other context in the input clearly suggests otherwise:
   - "morning" -> 9:00 AM
